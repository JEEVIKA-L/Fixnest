...
export const FilterDropdown = ({ onClose }: FilterDropdownProps) => {
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 mt-2 top-full w-80 bg-white border border-[#E5E7EB] rounded-2xl shadow-2xl z-[100] overflow-hidden flex flex-col max-h-[80vh]"
    >
      <div className="p-4 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
        <h3 className="text-xs font-black text-[#1F2937] uppercase tracking-widest">Filters</h3>
        {isActive() && (
          <button 
      ...
      <div className="p-4 space-y-6 overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input 
              autoFocus
              className="w-full bg-[#F6F7FB] border border-transparent focus:border-[#6A3DE8] rounded-xl px-4 py-2 text-sm focus:outline-none transition-all pr-10"
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 transition-all ${
                    isSelected 
                      ...
                      : 'border-[#F3F4F6] bg-[#F3F4F6] text-gray-500 hover:border-gray-200'
                  }`}
                >
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between p-2 rounded-xl border-2 transition-all text-left ${
                    isSelected 
                      ? 'border-[#6A3DE8] bg-white shadow-sm ring-1 ring-[#6A3DE8]/10' 
                      : 'border-transparent bg-gray-50 hover:bg-gray-100'
                  }`}
                >
                  <div className="flex items-center gap-2 truncate">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: label.color }}></div>
                    <span className="text-[11px] font-bold text-gray-700 truncate">{label.title}</span>
                  </div>
