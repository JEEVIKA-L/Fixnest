...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="h-20 flex items-center justify-between px-10 fixed top-0 w-full bg-white/70 backdrop-blur-md z-50 border-b border-gray-100">
        <Link href="/" className="flex items-center gap-2 text-2xl font-black text-[#1F2937]">
          <div className="bg-gradient-to-br from-[#8E2DE2] to-[#4A00E0] p-1.5 rounded-lg text-white">
            <Layout size={24} />
          </div>
          FixNest
        </Link>
        <div className="flex items-center gap-8 font-semibold text-sm text-gray-500">
      ...
      <section className="pt-40 pb-24 px-10 max-w-7xl mx-auto text-center">
        <motion.div
           ...
           animate="show"
        >
          <motion.div variants={item} className="inline-flex items-center gap-2 bg-[#F6F7FB] px-4 py-1.5 rounded-full text-[#6A3DE8] font-bold text-xs uppercase tracking-widest mb-6 border border-[#6A3DE8]/10 shadow-sm">
            <Star size={14} fill="currentColor" />
            Voted #1 Task Management Tool
          </motion.div>
          
          <motion.h1 variants={item} className="text-7xl font-black text-[#1F2937] leading-[1.1] mb-8">
            Organize work, <br />
          ...
          <motion.div variants={item} className="flex items-center justify-center gap-4">
            ...
            <button className="bg-white text-[#1F2937] border-2 border-gray-100 px-8 py-4 rounded-xl font-bold text-lg hover:border-gray-200 transition-all">
               Watch Demo
            </button>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          <div className="bg-[#F6F7FB] border border-gray-200 rounded-2xl p-4 shadow-2xl relative">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 h-[500px] flex overflow-hidden">
               ...
               <div className="flex-1 p-6 flex gap-6 overflow-hidden">
                   {[1, 2, 3].map(i => (
                     <div key={i} className="w-72 flex-shrink-0 bg-gray-50/50 border border-gray-100 rounded-xl p-4">
                        ...
                        <div className="space-y-3">
                          {[1, 2, 3].map(j => (
                            <div key={j} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                               <div className="w-full h-3 bg-gray-100 rounded mb-3"></div>
                               <div className="w-2/3 h-3 bg-gray-50 rounded mb-4"></div>
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <div key={i} className="space-y-4">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-[#6A3DE8] shadow-sm border border-gray-100">
                  {feature.icon}
                </div>
